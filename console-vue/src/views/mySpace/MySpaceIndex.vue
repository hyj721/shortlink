    ...
    <div class="content-box">
      <div class="table-box">
        ...
        <el-table :data="tableData" height="calc(100vh - 240px)" style="width: 100%"
          ...
          <el-table-column label="有效期" prop="validDate" min-width="150">
            <template #default="scope">
              <div v-if="scope.row.validDateType === 0">
                <el-tag type="success" effect="light" size="small" round>永久有效</el-tag>
              </div>
              <div v-else class="date-cell">
                <span class="date-text">{{ scope.row.validDate }}</span>
                <span v-if="!isExpire(scope.row.validDate)" class="status-badge expired">已失效</span>
                <span v-else class="status-badge active">生效中</span>
              </div>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="isAddGroup" title="新建短链接分组" style="width: 40%">
      <el-form :model="form" @submit.native.prevent>
        <el-form-item label="分组名称：" :label-width="formLabelWidth">
          <el-input autocomplete="off" v-model="newGroupName" placeholder="请输入分组名称" @keyup.enter="addGroup" />
        </el-form-item>
      </el-form>
      <template #footer>
...
.options-box {
  ...
  .item-box {
    height: 48px; 
    width: calc(100% - 16px); /* 自动计算宽度：100% 减去左右一共 16px 的边距 */
    margin: 4px 8px;
    border-radius: 6px;
...
.content-box {
  flex: 1;
  padding: 0; /* 移除内边距，铺满全屏 */
  background-color: #ffffff; /* 与表格背景一致，消除割裂感 */
  position: relative;
